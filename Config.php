	...
	protected function assetsList(): void
	{
		$this->addAsset('config', 'config.php', function () {
			...
			$example = "\n/*\n"
				...
				. " *         'searchable' => true,            // optional: item picker uses page-builder/search\n"
				. " *         'labelField' => ['name'],        // optional string or array used for item labels\n"
				. " *         'filters' => ['stars', 'destination' => ['label' => 'Destinazione', 'source' => 'destinations']],  // optional: fields the author may equality-filter list bindings on; 'options' => [val => label] renders a fixed dropdown, 'source' links the filter to another source's item picker (the column must hold that source's id; wins over options)\n"
				. " *         // 'fields' optional: auto-introspected from metadata if omitted\n"
				. " *     ],\n"
				. " *     'custom' => [\n"
				. " *         'retriever' => function (array \$filters, ?int \$limit = null) { return []; },  // returns items; \$filters may contain 'q', 'id', or author list-filters (apply them yourself)\n"
