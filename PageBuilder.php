...
class PageBuilder extends Module
{
	...
	/**
	 * Handle an editor image upload (the `image` config widget's onUploadImage).
	 * Moves $_FILES['upload'] under the configured `upload-path`, optimizes
	 * raster images through \Model\Images\Image, and returns the public URL.
	 * Throws on any failure — the controller (page-builder/upload) turns that into
	 * the 500 + { error: { message } } shape the editor expects. The disk base is
	 * anchored on this module's dir (doc-root `app/`), not CWD, so it's invariant
	 * to how the request is dispatched.
	 */
	public function uploadImage(): string
	{
		...
		$diskPath = __DIR__ . DIRECTORY_SEPARATOR . '..' . DIRECTORY_SEPARATOR . '..' . DIRECTORY_SEPARATOR . str_replace('/', DIRECTORY_SEPARATOR, $uploadPath);
		if (!is_dir($diskPath))
			mkdir($diskPath, 0777, true);

		if ($_FILES['upload']['error'] != UPLOAD_ERR_OK or !is_uploaded_file($_FILES['upload']['tmp_name']))
		...
		$host = '';
		if ($config['include-host-in-uploads'] ?? false) {
			if (!defined('HTTPS')) {
		...
		return $host . PATH . $uploadPath . $filename . '.' . $ext;
	}
